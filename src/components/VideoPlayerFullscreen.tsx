...
export const VideoPlayerFullscreen: React.FC<VideoPlayerFullscreenProps> = ({
  ...
  const updateProgressToDatabase = useCallback(async (progressPercent: number) => {
    ...
    try {
      ...
      await EmployeeService.updateVideoProgressByEmail(user.email, videoId, progressPercent, completedAt);
  ...
  const content = useMemo(() => {
    ...
    if (videoUrl && isYouTubeUrl(videoUrl)) {
      ...
      if (id) {
        return <iframe src={`https://www.youtube.com/embed/${id}?enablejsapi=1&origin=${window.location.origin}`} title={video.title} className="w-full h-full" allowFullScreen allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" onLoad={() => {
    ...
    if (videoUrl && isGoogleDriveUrl(videoUrl)) {
      ...
      if (embedUrl) {
        return <iframe src={embedUrl} title={video.title} className="w-full h-full" allowFullScreen onLoad={() => {
          // Clean up any existing interval  
          if (progressIntervalRef.current) {
            clearInterval(progressIntervalRef.current);
          }
    ...
    return <video ref={videoRef} className="w-full h-full object-fill" controls preload="metadata" onTimeUpdate={handleVideoProgress} onEnded={handleVideoEnded}>
        ...
        <DialogHeader className="pb-4 border-b flex-shrink-0">
          <div className="flex items-center justify-between">
            <div>
              ...
              {video?.description && video.description.trim() && <div className="pt-2 pb-1">
          ...
          <div className="flex items-center gap-2 mt-3">
            {!isCompleted && <>
                <Button variant={isWatching ? "default" : "outline"} size="sm" onClick={handleWatchingToggle} className="flex items-center gap-2">
                  {isWatching ? "📺 Watching" : "▶️ Start Watching"}
                </Button>
                
                {progress >= 10 && <Button variant="default" size="sm" onClick={handleMarkComplete} className="flex items-center gap-2 bg-green-600 hover:bg-green-700">
