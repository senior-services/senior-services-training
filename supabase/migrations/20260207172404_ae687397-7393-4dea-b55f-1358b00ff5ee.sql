-- Migration 1: Backfill existing employee names from profiles
UPDATE public.employees e
SET full_name = p.full_name,
    updated_at = now()
FROM public.profiles p
WHERE lower(e.email) = lower(p.email)
  AND (e.full_name IS NULL OR e.full_name = '')
  AND p.full_name IS NOT NULL
  AND p.full_name <> '';

-- Migration 2: Update handle_new_user to fill missing employee names on sign-up
