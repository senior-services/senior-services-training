-- Part A: Fix race condition in BOTH overloads of update_video_progress_by_email
-- Add COALESCE guard so completed_at is never overwritten with NULL once set

-- Fix the 4-parameter overload
CREATE OR REPLACE FUNCTION public.update_video_progress_by_email(p_email text, p_video_id uuid, p_progress_percent integer, p_completed_at timestamp with time zone DEFAULT NULL::timestamp with time zone)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- Fix the 6-parameter overload
CREATE OR REPLACE FUNCTION public.update_video_progress_by_email(p_email text, p_video_id uuid, p_progress_percent integer, p_completed_at timestamp with time zone DEFAULT NULL::timestamp with time zone, p_presentation_acknowledged_at timestamp with time zone DEFAULT NULL::timestamp with time zone, p_acknowledgment_viewing_seconds integer DEFAULT NULL::integer)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- Also fix the base update_video_progress function
CREATE OR REPLACE FUNCTION public.update_video_progress(p_employee_id uuid, p_video_id uuid, p_progress_percent integer, p_completed_at timestamp with time zone DEFAULT NULL::timestamp with time zone)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
